...
for (j in length(max_cryptos):1) {
  plot(lines(zoo(mean_sd_list[[j]][, 1]), type = "l", col = color[j], lwd = 3), ylim = c(-0.015, 0.025), 
       auto.grid = FALSE, main = "Means in rolling windows", 
       ylab = "mean log returns", minor.ticks = FALSE)
  dev.off()
  plot(lines(zoo(mean_sd_list[[j]][, 2]), type = "l", col = color[j], lwd = 3), ylim = c(-0.015, 0.025), 
       auto.grid = FALSE, main = "Means in rolling windows", 
       ylab = "mean log returns", minor.ticks = FALSE)
  dev.off()
  plot(lines(zoo(mean_sd_list[[j]][, 3]), type = "l", col = color[j], lwd = 3), ylim = c(-0.015, 0.025), 
       auto.grid = FALSE, main = "Means in rolling windows", 
       ylab = "mean log returns", minor.ticks = FALSE)
  dev.off()
  plot(lines(zoo(mean_sd_list[[j]][, 4]), type = "l", col = color[j], lwd = 3), ylim = c(-0.015, 0.025), 
       auto.grid = FALSE, main = "Means in rolling windows", 
       ylab = "mean log returns", minor.ticks = FALSE)
  dev.off() 
}
